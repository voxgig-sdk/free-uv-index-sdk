...
[MCP](https://modelcontextprotocol.io) server exposing the FreeUvIndex SDK as
two agent tools — `free-uv-index_list` and `free-uv-index_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-uv-index-mcp)
make build

# 2. Provide credentials via the environment
export FREE_UV_INDEX_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-uv-index \
  -- /absolute/path/to/free-uv-index-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-uv-index-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-uv-index_list: first page of records
{ "entity": "uvi" }
{ "entity": "uvi", "query": { } }

// free-uv-index_load: one record by id
{ "entity": "uvi", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-uv-index-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-uv-index \
     -- "$PWD"/dist/*/free-uv-index-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-uv-index_list` and `free-uv-index_load` tools now appear
   in new sessions. Ask the agent to *"list uvi using free-uv-index"*
   and it calls `free-uv-index_list` with `{"entity":"uvi"}`.
...
```sh
export FREE_UV_INDEX_APIKEY=sk_live_xxx            # API key
export FREE_UV_INDEX_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `free-uv-index_list` tool
...
### Call the `free-uv-index_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `free-uv-index_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-uv-index_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_UV_INDEX_APIKEY` | API key sent with every request. |
| `FREE_UV_INDEX_BASE` | Optional override of the API base URL. |
...
```sh
./free-uv-index-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-uv-index_load","arguments":{"entity":"uvi","query":{"id":1}}}}'
```
