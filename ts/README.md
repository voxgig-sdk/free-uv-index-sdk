# FreeUvIndex TypeScript SDK



The TypeScript SDK for the FreeUvIndex API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Uvi()` — each with a small set of operations (`list`)
...
`list()` resolves to an array of Uvi ENTITIES — every operation
resolves to entities, not raw records. Iterate them directly, and call
`.data()` on one for the record it holds:

```ts
const uvis = await client.Uvi().list({ latitude: 1, longitude: 1 })
...
const uvi = await client.Uvi().list()
// uvi is the entity, populated with mock response data
// — call uvi.data() for the record itself
console.log(uvi)
```
...
| `tester(testopts?, sdkopts?)` | `FreeUvIndexSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `FreeUvIndexSDK.test(testopts?, sdkopts?)` | `FreeUvIndexSDK` | Create a test-mode client. |
...
| `client` | `client(): FreeUvIndexSDK` | Return the parent SDK client. |
...
free-uv-index/
├── src/
│   ├── FreeUvIndexSDK.ts        # Main SDK class
