...
//     (\`new FreeUvIndexSDK(...)\` / \`FreeUvIndexSDK.test(...)\` become
//     \`FreeUvIndexSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"uvi":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreeUvIndexSDK(...)\` and any
// \`FreeUvIndexSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeUvIndexSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeUvIndexSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
