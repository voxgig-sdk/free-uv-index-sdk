...
from freeuvindex_sdk.freeuvindex_types import (
    Uvi,
    UviListMatch,
)
...
class UviEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return UviEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Uvi]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Uvi().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
