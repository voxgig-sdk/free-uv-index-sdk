# FreeUvIndex Python SDK



The Python SDK for the FreeUvIndex API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Uvi()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/free-uv-index-sdk/releases)) or
...
```python
try:
    uvis = client.Uvi().list({"latitude": 1, "longitude": 1})
    for uvi in uvis:
        print(uvi)
...
```
py/
├── freeuvindex_sdk.py         -- Main SDK module
