# FreeUvIndex Python SDK Reference

Complete API reference for the FreeUvIndex Python SDK.
...
#### `FreeUvIndexSDK.test(testopts=None, sdkopts=None)`
...
Create a new `UviEntity` instance. Pass `None` for no initial data.
...
```python
results = client.Uvi().list({"latitude": 1, "longitude": 1})
for uvi in results:
    print(uvi)
```
...
Create a new `UviEntity` instance with the same options.
