# FreeUvIndex SDK test runner

from __future__ import annotations
import os
import json

from freeuvindex_sdk.utility.voxgig_struct import voxgig_struct as vs


class FreeUvIndexTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeUvIndexTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeUvIndexTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeUvIndexTestRunner.getenv("FREE_UV_INDEX_TEST_LIVE")
        override = FreeUvIndexTestRunner.getenv("FREE_UV_INDEX_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeUvIndexTestRunner.getenv(key)
        ...
        explain = FreeUvIndexTestRunner.getenv("FREE_UV_INDEX_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FreeUvIndexTestRunner._test_control is not None:
            return FreeUvIndexTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeUvIndexTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeUvIndexTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeUvIndexTestRunner.load_test_control()
...
def load_env_local():
    FreeUvIndexTestRunner.load_env_local()


def env_override(m):
    return FreeUvIndexTestRunner.env_override(m)


def entity_data(v):
    return FreeUvIndexTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeUvIndexTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeUvIndexTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeUvIndexTestRunner.load_test_control()


def live_client_options():
    return FreeUvIndexTestRunner.live_client_options()


def live_delay_ms():
    return FreeUvIndexTestRunner.live_delay_ms()
