...
boru-driven command-line client **and** interactive REPL for the FreeUvIndex
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-uv-index-cli)
make build

# 2. See usage (words, entities, env vars)
./free-uv-index-cli --help

# 3. Provide credentials once, via the environment
export FREE_UV_INDEX_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./free-uv-index-cli list uvi

# 5. Override the API base URL for a single call
FREE_UV_INDEX_BASE=https://api.example.com ./free-uv-index-cli list uvi

# 6. No arguments -> interactive REPL
./free-uv-index-cli
free-uv-index> list uvi
free-uv-index> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-uv-index-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-uv-index-cli list uvi
   ```
...
Run with no arguments to open a REPL (prompt `free-uv-index>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `uvi`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-uv-index-cli`. |
...
(`./free-uv-index-cli <expr>`) and interactively (the REPL), and expressions compose the
