-- FreeUvIndex SDK Uvi entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local UviEntity = {}
UviEntity.__index = UviEntity


function UviEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UviEntity)
  self._name = "uvi"
  self._client = client
...
function UviEntity:get_name()
  return self._name
end


function UviEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UviEntity.new(self._client, opts)
end
...
function UviEntity:mark_deleted()
  self._deleted = true
end


function UviEntity:deleted()
  return true == self._deleted
end


function UviEntity:data_set(args)
  if args ~= nil then
...
function UviEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UviEntity:match_set(args)
  if args ~= nil then
...
function UviEntity:stream(action, args, callopts)
...
---@param reqmatch UviListMatch
---@param ctrl? table
---@return Uvi[]
---@return string? err
function UviEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function UviEntity:_run_op(ctx, post_done)
  local utility = self._utility
