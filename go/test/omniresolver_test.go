...
import (
	...
	omni "github.com/voxgig-sdk/free-uv-index-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeUvIndexError); is {
				out := map[string]any{
					"name":    "FreeUvIndexError",
...
func omniCtx(arg any, client *sdk.FreeUvIndexSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeUvIndexSDK); is {
			client = live
