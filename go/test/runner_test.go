...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FREE_UV_INDEX_TEST_LIVE") == "TRUE" ||
		os.Getenv("FREE_UV_INDEX_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FREE_UV_INDEX_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.FreeUvIndexSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.FreeUvIndexError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.FreeUvIndexSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.FreeUvIndexError{Msg: msg, Code: code}
}
