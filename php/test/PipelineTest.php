...
{
    private static function utility(): FreeUvIndexUtility
    {
        return new FreeUvIndexUtility();
    }
    ...
    private static function ctx(array $over = []): FreeUvIndexContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FreeUvIndexContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FreeUvIndexOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FreeUvIndexError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FreeUvIndexOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FreeUvIndexMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FreeUvIndexOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FreeUvIndexMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FreeUvIndexOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FreeUvIndexMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeUvIndexSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FreeUvIndexMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FreeUvIndexResponse([]),
            'result' => new FreeUvIndexResult([]),
        ]);
        [, $err] = FreeUvIndexMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FreeUvIndexSpec([]),
            'response' => null,
            'result' => new FreeUvIndexResult([]),
        ]);
        [, $err] = FreeUvIndexMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FreeUvIndexSpec(['step' => 's']),
            'response' => new FreeUvIndexResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FreeUvIndexResult([]),
        ]);
        [, $err] = FreeUvIndexMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeUvIndexSpec(['step' => 's']),
            'response' => new FreeUvIndexResponse(self::resp(200, ['v' => 1])),
            'result' => new FreeUvIndexResult([]),
        ]);
        [, $err] = FreeUvIndexMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeUvIndexSpec(['step' => 's']),
            'response' => new FreeUvIndexResponse(self::resp(200, ['v' => 2])),
            'result' => new FreeUvIndexResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FreeUvIndexMakeResponse::call($ctx);
    ...
    {
        $preset = new FreeUvIndexResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FreeUvIndexSpec([]),
            'response' => new FreeUvIndexResponse([]),
            'result' => new FreeUvIndexResult([]),
        ...
        [$got, $err] = FreeUvIndexMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FreeUvIndexResult([])]);
        [, $err] = FreeUvIndexMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FreeUvIndexSpec([]), 'result' => null]);
        [, $err] = FreeUvIndexMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeUvIndexSpec(['step' => 's']),
            'result' => new FreeUvIndexResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FreeUvIndexOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeUvIndexMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeUvIndexSpec(['step' => 's']),
            'result' => new FreeUvIndexResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FreeUvIndexOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeUvIndexMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeUvIndexResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FreeUvIndexSpec([]),
            'result' => new FreeUvIndexResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FreeUvIndexMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeUvIndexMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FreeUvIndexError('boom', 'boom');
        $utility->fetcher = function (FreeUvIndexContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeUvIndexSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FreeUvIndexMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FreeUvIndexContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FreeUvIndexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FreeUvIndexResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        FreeUvIndexMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FreeUvIndexContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeUvIndexSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FreeUvIndexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FreeUvIndexResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FreeUvIndexSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FreeUvIndexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FreeUvIndexMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FreeUvIndexMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FreeUvIndexResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FreeUvIndexDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FreeUvIndexResult(['ok' => false])]);
        $this->expectException(FreeUvIndexError::class);
        FreeUvIndexDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FreeUvIndexResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FreeUvIndexMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FreeUvIndexMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FreeUvIndexBaseFeature();
        $b = new FreeUvIndexBaseFeature();
        FreeUvIndexFeatureAdd::call($ctx, $a);
        FreeUvIndexFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FreeUvIndexBaseFeature
    {
        $f = new FreeUvIndexBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FreeUvIndexFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FreeUvIndexContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FreeUvIndexSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeUvIndexPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        FreeUvIndexPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FreeUvIndexResponse(['headers' => null]),
            'result' => new FreeUvIndexResult([]),
        ]);
        FreeUvIndexResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        FreeUvIndexResultBody::call($ctx);
