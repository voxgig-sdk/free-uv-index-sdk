...
The PHP SDK for the FreeUvIndex API — an entity-oriented client using PHP conventions.
...
$uvi = $client->Uvi()->list();
print_r(array_map(fn($item) => $item->data_get(), $uvi));
```
...
```php
require_once 'freeuvindex_sdk.php';
$client = new FreeUvIndexSDK($options);
```
...
```
php/
├── freeuvindex_sdk.php          -- Main SDK class
...
The main class (`freeuvindex_sdk.php`) exports the SDK class
