...
class FreeUvIndexSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FreeUvIndexHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FreeUvIndexHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FreeUvIndexFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FreeUvIndexFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FreeUvIndexUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FreeUvIndexHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FreeUvIndexHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FreeUvIndexHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FreeUvIndexError($op . "_allow",
                "FreeUvIndexSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FreeUvIndexHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FreeUvIndexError("graphql_error",
                "FreeUvIndexSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Uvi($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_uvi === null) {
                $this->_uvi = new UviEntity($this, null);
            }
            return $this->_uvi;
        }
        return new UviEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new FreeUvIndexSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
