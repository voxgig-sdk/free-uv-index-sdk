...
class UviDirectTest < Minitest::Test
  def test_direct_list_uvi
    setup = uvi_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-uvi", setup[:live] ? "live" : "unit")
    if _should_skip
...
def uvi_direct_setup(mockres)
  ...
  if live
    ...
    client = FreeUvIndexSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
