...
module FreeUvIndexOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if FreeUvIndexOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = FreeUvIndexOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| FreeUvIndexOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || FreeUvIndexOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = FreeUvIndexSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = FreeUvIndexError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { FreeUvIndexOmni.errify(err) }
