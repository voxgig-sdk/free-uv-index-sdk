...
class UviEntity
  ...
  def make
    opts = @_entopts.dup
    UviEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FreeUvIndexHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FreeUvIndexHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [UviListMatch, Hash, nil] match filter (any subset of
  #   Uvi fields); defaults to nil, treated as an empty match that lists all.
