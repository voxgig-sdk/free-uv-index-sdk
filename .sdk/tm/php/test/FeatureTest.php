...
class FtCtrl extends FreeUvIndexControl
{
...
class FtClient
{
    ...
    private FreeUvIndexUtility $utility;

    public function __construct(FreeUvIndexUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeUvIndexUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeUvIndexContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreeUvIndexContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreeUvIndexContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeUvIndexConfig::make_feature($name);
    ...
    private static function buildUrl(FreeUvIndexSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeUvIndexError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeUvIndexResponse($fetched);
        }
    ...
    private function fail(FreeUvIndexContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeUvIndexContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
