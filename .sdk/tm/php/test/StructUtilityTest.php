...
// FreeUvIndex SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = FreeUvIndexOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(FreeUvIndexOmni::structfix($now, Struct::undef()))
                        !== json_encode(FreeUvIndexOmni::structfix($val, Struct::undef()))) {
