...
module FreeUvIndexUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreeUvIndexResponse.new({})
    ...
    if fetch_err
      ...
      response = FreeUvIndexResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeUvIndexResponse.new(fetched)
    else
